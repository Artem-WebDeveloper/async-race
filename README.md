...
Самые интересные задачи в реализации - конкурентное управление заездом: все двигатели стартуют одновременно через `Promise.all`, победитель определяется через `Promise.any`. При обрыве соединения (500 от API) анимация фиксируется через `getComputedStyle` - машина останавливается в точке обрыва, не прыгая назад. Стор реализует Observer pattern вручную: два независимых канала подписки для Garage и Winners, страницы перерисовываются реактивно при изменении стейта.
